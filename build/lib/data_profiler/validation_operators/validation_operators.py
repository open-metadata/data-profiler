...
from data_profiler.checkpoint.util import send_slack_notification
from data_profiler.data_asset import DataAsset
from data_profiler.data_asset.util import parse_result_format
...
from data_profiler.validation_operators.types.validation_operator_result import (
...
class ActionListValidationOperator(ValidationOperator):
    ...
    Each action in the list must be an instance of :py:class:`ValidationAction<data_profiler.validation_operators.actions.ValidationAction>`
    class (or its descendants). See the actions included in Great Expectations and how to configure them :py:mod:`here<data_profiler.validation_operators.actions>`. You can also implement your own actions by extending the base class.
    ...
    **Configuration**
    ...
    An instance of ActionListValidationOperator is included in the default configuration file ``data_profiler.yml`` that ``data_profiler init`` command creates.
        ...
        action_list:
          ...
          - name: send_slack_notification_on_validation_result
            action:
              ...
              renderer:
                module_name: data_profiler.render.renderer.slack_renderer
    ...
    * ``assets_to_validate`` - an iterable that specifies the data assets that the operator will validate. The members of the list can be either batches or triples that will allow the operator to fetch the batch: (data_asset_name, expectation_suite_name, batch_kwargs) using this method: :py:meth:`~data_profiler.data_context.BaseDataContext.get_batch`
