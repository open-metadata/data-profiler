...
def build_store_from_config(
    ...
    module_name: str = "data_profiler.data_context.store",
...
def substitute_value_from_aws_secrets_manager(value):
    ...
    if not boto3:
        logger.error(
            "boto3 is not installed, please install data_profiler with aws_secrets extra > "
            "pip install data_profiler[aws_secrets]"
        )
...
def substitute_value_from_gcp_secret_manager(value):
    ...
    if not secretmanager:
        logger.error(
            "secretmanager is not installed, please install data_profiler with gcp extra > "
            "pip install data_profiler[gcp]"
        )
...
def substitute_value_from_azure_keyvault(value):
    ...
    if not SecretClient:
        logger.error(
            "SecretClient is not installed, please install data_profiler with azure_secrets extra > "
            "pip install data_profiler[azure_secrets]"
        )
